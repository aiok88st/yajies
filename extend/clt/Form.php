...
class Form{
    ...
    public function catid($info,$value){
        ...
        $list = db('category')->order('listorder asc')->select();;
    ...
    public function title($info,$value){
        ...
        //标题颜色及是否加粗
        $stylestr .="<div style='display: none;'>";
    ...
    public function textarea($info,$value){
        ...
        $parseStr   = '<textarea data-required="'.$info['required'].'" min="'.$info['minlength'].'" max="'.$info['maxlength'].'" errormsg="'.$info['errormsg'].'" title="'.$name.'" placeholder="请输入'.$name.'" lay-verify="defaul'.$pattern.'"  class="'.$info['class'].' layui-textarea" name="'.$field.'" />'.stripslashes($value).'</textarea>';
        if($info['required']==1){
            $parseStr .='</div>';
            $parseStr .='<div class="layui-form-mid layui-word-aux red">*必填';
        }
    ...
    public function editor($info,$value){
        ...
        if($info['setup']['edittype']=='UEditor'){
            ...
            $str .='<textarea name="'.$field.'" class="'.$info['class'].'" id="'.$info['class'].'" style="height:500px;">'.$value.'</textarea>';
